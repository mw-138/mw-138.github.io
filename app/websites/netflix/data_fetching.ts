const Key = "596618f23f3e23501897e02979f122ed";
...
const Requests = {
  ...
  NowPlaying: `https://api.themoviedb.org/3/discover/movie?api_key=${Key}&language=en-US&page=1`,
  TopRated: `https://api.themoviedb.org/3/discover/movie?api_key=${Key}&include_adult=false&include_video=false&language=en-US&page=1&sort_by=vote_average.desc&without_genres=99,10755&vote_count.gte=200`,
};
...
export async function getUpcomingMovies(params?: string): Promise<Movie[]> {
  ...
  const res = await performMovieQuery(
    `https://api.themoviedb.org/3/movie/upcoming?api_key=${Key}&include_adult=false&include_video=false&language=en-US&page=1&with_release_type=4|3&release_date.gte={${today}}&release_date.lte={${max}}${params ?? ""}`,
  );
